...
public class ZoomVerticalScrollView extends ScrollView implements ScaleGestureDetector.OnScaleGestureListener {
    private enum Mode {
        NONE,
        DRAG,
        ZOOM
    ...
    public ZoomVerticalScrollView(Context context) {
        super(context);
        init(context);
    }

    public ZoomVerticalScrollView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ZoomVerticalScrollView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private Rect getBounds(){
        ...
        float maxY = ((child().getHeight() /2.0f)* scale) - (child().getHeight() /2.0f);
        float minX = maxX - (child().getWidth() * scale - (rect.right - rect.left));
        float minY = maxY - (child().getHeight() * scale - (rect.bottom - rect.top));
