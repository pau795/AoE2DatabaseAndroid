...
public abstract class Item extends Entity {
    ...
    protected OnTypeValueChanged attackListener, armorListener;
    ...
    public void setOnAttackChangedListener(OnTypeValueChanged listener){
        this.attackListener = listener;
    }

    public void setOnArmorChangedListener(OnTypeValueChanged listener){
        this.armorListener = listener;
    }

    //PREVIEW
    public EntityElement getClassElement(){
        return getEntityElement(Database.getAppContext().getString(R.string.entity_class));
    }

    public EntityElement getPreviousUpgradeElement(){
        return getEntityElement(Database.getAppContext().getString(R.string.upgraded_from));
    }
    ...
    public void processPostBonus() {
        ...
        if (attackListener != null) attackListener.onChange();
        if (armorListener != null) armorListener.onChange();
    }
    ...
    private LinkedHashMap<String, List<TypeElement>> processTypeValues(LinkedHashMap<String, LinkedHashMap<Integer, Double>> m1,
        ...
        if (r.isEmpty()) r.put(Database.getAppContext().getString(R.string.none), new ArrayList<TypeElement>());
        return r;
    }
